...
async function createTransaction() {
  ...
  // Get the signed transaction from the external signer
...
async function getSignedTransactionFromExternalSigner(
  ...
  const thePrivateKey = devWallet.privateKey; 
  const theWallet = new ethers.Wallet(thePrivateKey, provider);
  
  // Deserialize the transaction
  const unsignedTx = ethers.Transaction.from(serializedUnsignedTx);
  
  // Sign the transaction
  const signedTx = await theWallet.signTransaction(unsignedTx);
